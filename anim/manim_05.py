...
circle_color, circle_thick = GREEN, DEFAULT_STROKE_WIDTH
...
class manim_05(Scene):
    def trazar_circulo(self, circ: Circle, initial_angle = 0, run_time = 2.0, show_vect = True):
        orig_color = circ.get_color()
        center = circ.get_center()
        radius = circ.radius
        if(show_vect):
            vect = Arrow(start = center, end = center + [cos(initial_angle) * radius, sin(initial_angle) * radius, 0.0], buff = 0, color = aux_color, 
            ...
            self.play(
                Create(circ),
                Rotate(vect, 2*PI, about_point = circ.get_center()),
                run_time = run_time*6/8
            )
            self.play(Uncreate(vect), run_time = run_time/8)
        else:
            self.play(circ.animate(rate_func = rush_into).set_color(color = YELLOW_B), run_time = run_time/8)
            self.play(
                Create(circ),
                Rotate(vect, 2*PI, about_point = circ.get_center()),
                run_time = run_time*6/8
            )
            self.play(circ.animate(rate_func = rush_from).set_color(color = orig_color), run_time = run_time/8)
    
    def construct(self):
        # compilar con --save-sections para generar vídeos separados para cada sección
        
        # definiciones puntos base
        ...
        # parte 1 : intersección rectas
        line1 = Line(pA.get_center(), pB.get_center(), buff = 0, color = RED, stroke_width = line_thick)
        line2 = Line(pC.get_center(), pD.get_center(), buff = 0, color = BLUE, stroke_width = line_thick)
        self.play(AnimationGroup(FadeIn(pA), FadeIn(pB), FadeIn(pA_label), FadeIn(pB_label), lag_rate = 0.5))
        self.play(Create(line1))
        self.wait(1)
        self.play(AnimationGroup(FadeIn(pC), FadeIn(pD), FadeIn(pC_label), FadeIn(pD_label), lag_rate = 0.5))
        ...
        circle1 = Circle(arc_center = pC.get_center(), radius = 4, stroke_width = circle_thick, stroke_color = circle_color)
        self.play(Create(line1))
        self.wait(1)
        self.trazar_circulo(circle1)
        self.wait(3)
        ...
        self.wait(1)
        group2 = Group(line1, circle1, eq_line1, eq_circle1, eq_line1_plugged, eq_circle1_plugged, eq_pF, pF, pF_label)
