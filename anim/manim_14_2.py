...
class manim_14_2(Scene):
    def construct(self):
        ...
        e_pii = Tex(r"$e^{i\pi}$ es algebraico", font_size = fs*1.5, color = BLUE).move_to(ipi_alg.get_center())
        self.play(Transform(ipi_alg, e_pii), FadeOut(fact_1_copy))
        self.wait(2)
        fact_2_copy = fact_2.copy()
        self.play(fact_2_copy.animate.next_to(ipi_alg, UP * 1.2))
        self.play(fact_2_copy.animate.scale(1.6))
        cont = Tex(r"Contradicción!", font_size = fs*1.5, color = RED).move_to(ipi_alg.get_center())
        self.play(Transform(ipi_alg, cont), FadeOut(fact_2_copy))
        self.wait(2)