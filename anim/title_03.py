...
class title_03(Scene):
    def construct(self):    
        title = Tex("Sección", font_size = 90).shift(UP*1.5)
        number = Tex("3", font_size = 90, color = BLUE).next_to(title, RIGHT).align_to(title, DOWN)
        title_line = VGroup(title, number)
        line = Line([-3.0, 0.0, 0.0], [3.0, 0.0, 0.0], color = YELLOW).next_to(title_line, DOWN)
        subtitle = Tex("El álgebra de la regla y compás", font_size = 60).next_to(line, DOWN*1.5)
